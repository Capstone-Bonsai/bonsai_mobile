...
Future<List<WorkingDate>> fetchData(int month, int year) async {
  {
    ...
    final uri = Uri.parse('${ApiConfig.baseUrl}/ServiceOrder/WorkingCalendar?month=$month&year=$year');
...
Future<WorkingDetail> getWorkingDetail(String id) async {
  {
    try {
      var sharedPref = await SharedPreferences.getInstance();
      String? token = sharedPref.getString('token');
      final uri = Uri.parse('${ApiConfig.baseUrl}/ServiceOrder/Gardener/$id');
      final response = await http.get(
        uri,
        headers: {
          'Authorization': 'Bearer $token',
